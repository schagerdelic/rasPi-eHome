...
def debugWrite(cnt, s):
    try:
        fn = "/home/pi/homeputer/eHomeDebugLog.csv"
        fd = open(fn,"a")
...
if not(inetChk.isConnectionAlive("www.google.com")):
    debugWrite(globalCnt,'NETWORK CONNECTION LOST - DISABLE/ENABLE NETWORK ADAPTER')
...
while 1:
        ...
        try:
                ...
                try:
                    #TODO: get network config from web? # copy from web folder to local
                    __settings__ = eHomeSettings.eHomeSettings("/var/www/eHome/eHomeSettings.json")
                    #ret = os.popen('copy Q:\\eHome\\eHomeSettings.json C:\\Users\\eHome\\Documents\\homeputer\\')
                except Exception, err:
                    print "Q:\eHome... NOT FOUND... using C:\\"
                    __settings__ = eHomeSettings.eHomeSettings("/home/pi/homeputer/eHomeSettings.json")                        
                ...
                if (globalCnt % 30 == 0): # 3 = 1min, 15 = 5min
                    if not(inetChk.isConnectionAlive("www.schagerdelic.com")):
